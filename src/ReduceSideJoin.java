...
public class ReduceSideJoin {
    ...
    public static class DetailsMapper extends Mapper<LongWritable, Text, LongWritable, Text> {
        ...
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (input.length == 10) {
                ...
                String data = input[1] + "," + result + "," + input[3] + "," + input[4] + "," + input[5];
                outvalue.set("R:" + data);
                ...
                for (Text A : listA) {
                    ...
                    for (String s : friend) {
                        if (userData.containsKey(s)) {
                            String[] ageCal = userData.get(s).split(":");
                            Date curr = new Date();
                            int currMonth = curr.getMonth() + 1;
                            int currYear = curr.getYear() + 1900;
                            String[] cal = ageCal[2].split("/");
                            int result = currYear - Integer.parseInt(cal[2]);
                            if (Integer.parseInt(cal[0]) > currMonth)
                                result--;
                            else if (Integer.parseInt(cal[0]) == currMonth) {
                    ...
                    for (Text B : listB) {
                        ...
                        res.append(",");
                        res.append(new Text(new FloatWritable(avgAge / count).toString()));
                    }
                    C.set(res.toString());
                }
            }
    ...
    public static class MaxAgeMapper extends Mapper<LongWritable, Text, MaxAgeCustom, Text> {
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (m.length == 2) {
                ...
                context.write(new MaxAgeCustom(Float.parseFloat(m[0]), Float.parseFloat(line[5])), new Text(m[1]));
            }
        }
    }
    ...
    public static class MaxAgeCustom implements WritableComparable<MaxAgeCustom> {
        ...
        MaxAgeCustom(Float user, Float age) {
            this.userId = user;
            this.age = age;
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Path outputDirIntermediate1 = new Path(otherArgs[2] + "_int1");
        Path outputDirIntermediate2 = new Path(otherArgs[2]);
        conf.set("userdata", otherArgs[0]);
        if (otherArgs.length != 3) {
            System.err.println("Usage: ReduceSideJoin <UserData> <FriendsList > <Output>");
            System.exit(2);
        }
        ...
        MultipleInputs.addInputPath(job, new Path(otherArgs[0]), TextInputFormat.class, DetailsMapper.class);
        ...
        FileInputFormat.addInputPath(job1, new Path(String.valueOf(outputDirIntermediate1)));
